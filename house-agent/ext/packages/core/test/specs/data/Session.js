topSuite("Ext.data.Session", [
    ...
    var HouseAgentSystemGroup, peonGroup;
    var rufusGroups, billGroups, tedGroups;
    var HouseAgentSystemUsers, peonUsers;
    ...
    beforeEach(function() {
        ...
        HouseAgentSystemGroup = {
            id: 42,
            name: 'HouseAgentSystems'
        };
        peonGroup = {
            id: 427,
            name: 'Peons'
        ...
        rufusGroups = [ HouseAgentSystemGroup, peonGroup ];
        billGroups = [ peonGroup ];
        tedGroups = [ peonGroup ];

        HouseAgentSystemUsers = [ userRufus ];
    ...
    describe('Many-to-many associations', function () {
        ...
        describe("loading a many-to-many", function() {
            it('should load groups for a user', function () {
                ...
                expect(groups.getCount()).toBe(2);
                expect(groups.getById(HouseAgentSystemGroup.id)).toBeTruthy();
            ...
            it('should load both sides of a matrix', function () {
                var rufusGroupsStore = session.getRecord('User', userRufus.id, false).groups(),
                    HouseAgentSystemUsersStore = session.getRecord('Group', HouseAgentSystemGroup.id, false).users(),
                    peonUsersStore = session.getRecord('Group', peonGroup.id, false).users();

                rufusGroupsStore.load();
                peonUsersStore.load();
                HouseAgentSystemUsersStore.load();

                completeRequest(rufusGroups, 1);
                completeRequest(peonUsers, 2);
                completeRequest(HouseAgentSystemUsers, 3);

                expect(rufusGroupsStore.isStore).toBe(true);
                expect(rufusGroupsStore.getCount()).toBe(2);
                expect(rufusGroupsStore.getById(HouseAgentSystemGroup.id)).toBeTruthy();
                expect(rufusGroupsStore.getById(peonGroup.id)).toBeTruthy();

                var rufusRec1, rufusRec2;

                expect(HouseAgentSystemUsersStore.isStore).toBe(true);
                expect(HouseAgentSystemUsersStore.getCount()).toBe(1);
                expect(rufusRec1 = HouseAgentSystemUsersStore.getById(userRufus.id)).toBeTruthy();
            ...
            it('should allow editing on both sides of a matrix', function () {
                ...
                completeRequest(HouseAgentSystemUsers, 4);

                // Removing Rufus from the HouseAgentSystemUsersStore should reflexively remove
                // the HouseAgentSystemGroup from rufusGroupsStore.
                expect(rufusGroupsStore.getCount()).toBe(2);
                expect(rufusGroupsStore.getById(HouseAgentSystemGroup.id)).toBeTruthy();

                var rufusRec = HouseAgentSystemUsersStore.getById(userRufus.id);
                HouseAgentSystemUsersStore.remove(rufusRec);

                expect(rufusGroupsStore.getCount()).toBe(1);
                expect(rufusGroupsStore.getById(HouseAgentSystemGroup.id)).toBe(null);

                // Adding Bill to the HouseAgentSystemUsersStore should reflexively add HouseAgentSystemGroup
                // to billGroupsStore
                expect(billGroupsStore.getCount()).toBe(1);
                expect(billGroupsStore.getById(HouseAgentSystemGroup.id)).toBe(null);

                var billRec = peonUsersStore.getById(userBill.id);
                HouseAgentSystemUsersStore.add(billRec);

                expect(billGroupsStore.getCount()).toBe(2);
                expect(billGroupsStore.getById(HouseAgentSystemGroup.id)).toBeTruthy();
